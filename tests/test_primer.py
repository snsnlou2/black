...
FAKE_PROJECT_CONFIG = {'cli_arguments': ['--unittest'], 'expect_formatting_changes': False, 'git_clone_url': 'https://github.com/psf/black.git'}
...
async def raise_subprocess_error_1(*args: Any, **kwargs):
    raise CalledProcessError(1, ['unittest', 'error'], b'', b'')

async def raise_subprocess_error_123(*args: Any, **kwargs):
    raise CalledProcessError(123, ['unittest', 'error'], b'', b'')

async def return_false(*args: Any, **kwargs):
    return False

async def return_subproccess_output(*args: Any, **kwargs):
    return (b'stdout', b'stderr')

async def return_zero(*args: Any, **kwargs):
    return 0

class PrimerLibTests(unittest.TestCase):

    def test_analyze_results(self):
        fake_results = lib.Results({'disabled': 0, 'failed': 1, 'skipped_long_checkout': 0, 'success': 68, 'wrong_py_ver': 0}, {'black': CalledProcessError(69, ['black'], b"Black didn't work", b'')})
    ...
    @event_loop()
    def test_gen_check_output(self):
        loop = asyncio.get_event_loop()
        (stdout, stderr) = loop.run_until_complete(lib._gen_check_output([lib.BLACK_BINARY, '--help']))
        ...
        with self.assertRaises(asyncio.TimeoutError):
            loop.run_until_complete(lib._gen_check_output(['/bin/sleep', '2'], timeout=0.1))

    @event_loop()
    def test_git_checkout_or_rebase(self):
        loop = asyncio.get_event_loop()
        project_config = deepcopy(FAKE_PROJECT_CONFIG)
        work_path = Path(gettempdir())
        expected_repo_path = (work_path / 'black')
        with patch('black_primer.lib._gen_check_output', return_subproccess_output):
            returned_repo_path = loop.run_until_complete(lib.git_checkout_or_rebase(work_path, project_config))
        self.assertEqual(expected_repo_path, returned_repo_path)

    @patch('sys.stdout', new_callable=StringIO)
    @event_loop()
    def test_process_queue(self, mock_stdout):
        loop = asyncio.get_event_loop()
        config_path = (Path(lib.__file__).parent / 'primer.json')
        with patch('black_primer.lib.git_checkout_or_rebase', return_false):
            with TemporaryDirectory() as td:
                return_val = loop.run_until_complete(lib.process_queue(str(config_path), td, 2))
...
class PrimerCLITests(unittest.TestCase):
    ...
    def test_async_main(self):
        ...
        args = {'config': '/config', 'debug': False, 'keep': False, 'long_checkouts': False, 'rebase': False, 'workdir': str(work_dir), 'workers': 69}
