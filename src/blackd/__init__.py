...
try:
    ...
    print(((f'aiohttp dependency is not installed: {ie}. ' + "Please re-install black with the '[d]' extra install ") + 'to obtain aiohttp_cors: `pip install black[d]`'), file=sys.stderr)
    sys.exit((- 1))
import black
import click
...
DIFF_HEADER = 'X-Diff'
BLACK_HEADERS = [PROTOCOL_VERSION_HEADER, LINE_LENGTH_HEADER, PYTHON_VARIANT_HEADER, SKIP_STRING_NORMALIZATION_HEADER, FAST_OR_SAFE_HEADER, DIFF_HEADER]
...
def make_app():
    ...
    cors.add(resource.add_route('POST', partial(handle, executor=executor)), {'*': aiohttp_cors.ResourceOptions(allow_headers=(*BLACK_HEADERS, 'Content-Type'), expose_headers='*')})
    return app

async def handle(request, executor):
    ...
    try:
        ...
        if (PYTHON_VARIANT_HEADER in request.headers):
            ...
            try:
                ...
                return web.Response(status=400, text=f'Invalid value for {PYTHON_VARIANT_HEADER}: {e.args[0]}')
        else:
            pyi = False
            versions = set()
        ...
        if (request.headers.get(FAST_OR_SAFE_HEADER, 'safe') == 'fast'):
            fast = True
        mode = black.FileMode(target_versions=versions, is_pyi=pyi, line_length=line_length, string_normalization=(not skip_string_normalization))
        req_bytes = (await request.content.read())
        charset = (request.charset if (request.charset is not None) else 'utf8')
        req_str = req_bytes.decode(charset)
        then = datetime.utcnow()
        loop = asyncio.get_event_loop()
        formatted_str = (await loop.run_in_executor(executor, partial(black.format_file_contents, req_str, fast=fast, mode=mode)))
        ...
        if only_diff:
            ...
            loop = asyncio.get_event_loop()
            formatted_str = (await loop.run_in_executor(executor, partial(black.diff, req_str, formatted_str, src_name, dst_name)))
        return web.Response(content_type=request.content_type, charset=charset, headers=headers, text=formatted_str)
...
def parse_python_variant_header(value):
    if (value == 'pyi'):
        return (True, set())
    else:
        versions = set()
